...
class WorkerThread(QThread):
	...
	def run_receiver(self):
		...
		for line in iter(self.process.stdout.readline, ''):
			...
			# Detect ACK sent - Receiver doesn't *get* ACKs, it *sends* them.
			# We only visualize ACKs on the Sender side.
			# Receiver progress is based on received chunks.
			...
			if "[COMPLETE]" in line_stripped or "Reception complete" in line_stripped:
...
class SenderPanel(QWidget):
	...
	def setup_ui(self):
		...
		# --- Removed ACK visualization group ---
		# ack_group = ModernGroupBox("Acknowledgment Status")
		# ack_layout = QVBoxLayout()
		# self.ack_panel = AcknowledgmentPanel() # Removed
		# ack_layout.addWidget(self.ack_panel) # Removed
		# ack_group.setLayout(ack_layout) # Removed
		# main_layout.addWidget(ack_group) # Removed
