...
def send_file(file_path, target_ip=None, key_path=None, chunk_size=MAX_CHUNK_SIZE, delay=0.1):
    ...
    # If target IP is not provided, discover the receiver
    if not target_ip:
        print("[DISCOVERY] No target IP provided, initiating receiver discovery...")
        log_debug("No target IP provided, initiating receiver discovery")
        
        target_ip = discover_receiver(key_path)
        if not target_ip:
            log_debug("Failed to discover receiver")
            print("[DISCOVERY] Failed to discover receiver. Aborting transmission.")
            return False
            
        print(f"[DISCOVERY] Using discovered receiver IP: {target_ip}")
        log_debug(f"Using discovered receiver IP: {target_ip}")
...
def parse_arguments():
    ...
    parser.add_argument('--target', '-t', help='Target IP address (optional, will autodiscover if not provided)')
    ...
    parser.add_argument('--discovery-timeout', '-dt', type=int, default=30,
                        help='Timeout in seconds for receiver discovery (default: 30)')
